...
public class Common {
	...
	/**
	 * The directory name of the folder where temporary files are stored. The directory will be cleaned during the creation of the main activity (
	 * {@link }). (sub directory of {@link #HOME_DIR}.)
	 */
	 ...
	 */
	public static final String[] SOME_CLASSICAL_KNOWN_KEYS = { "000000000000", "A0B0C0D0E0F0", "A1B1C1D1E1F1", "B0B1B2B3B4B5", "4D3A99C351DD",
			"1A982C7E459A", "AABBCCDDEEFF" };
	...
	/**
	 * Read a file line by line. The file should be a simple text file. Empty lines and lines STARTING with "#" will not be interpreted.
	 * 
	 * @param file The file to read.
	 * @param readComments Whether to read comments or to ignore them. Comments are lines STARTING with "#" (and empty lines).
	 * @return Array of strings representing the lines of the file. If the file is empty or an error occurs "null" will be returned.
	 */
	...
	/**
	 * Write an array of strings (each field is one line) to a given file. If the file already exists, it will be overwritten.
	 * 
	...
	public static void enableNfcForegroundDispatch(Activity targetActivity) {
		if (mNfcAdapter != null && mNfcAdapter.isEnabled()) {
			...
			mNfcAdapter.enableForegroundDispatch(targetActivity, pendingIntent, null, new String[][] { new String[] { NfcA.class.getName() } });
		}
	}
...
//			Toast.makeText(context, id, Toast.LENGTH_LONG).show();
	 ...
	 * {@link com.nfc_mifareclassic.api.daum.library.nfcdemo.Common.Operations}) you asked for.<br />
	 ...
	 *         </ul>
	 */
	public static int getOperationInfoForBlock(byte c1, byte c2, byte c3, Operations op, boolean isSectorTrailer, boolean isKeyBReadable) {
		// Is Sector Trailer?
		if (isSectorTrailer) {
			// Sector Trailer.
			if (op != Operations.ReadKeyA && op != Operations.ReadKeyB && op != Operations.ReadAC && op != Operations.WriteKeyA
					&& op != Operations.WriteKeyB && op != Operations.WriteAC) {
				// Error. Sector Trailer but no Sector Trailer permissions.
				return 4;
			}
			if (c1 == 0 && c2 == 0 && c3 == 0) {
				if (op == Operations.WriteKeyA || op == Operations.WriteKeyB || op == Operations.ReadKeyB || op == Operations.ReadAC) {
					return 1;
				}
				return 0;
			...
			// Data Block.
			if (op != Operations.Read && op != Operations.Write && op != Operations.Increment && op != Operations.DecTransRest) {
	 ...
	 * @return Matrix of access conditions bits (C1-C3) where the first dimension is the "C" parameter (C1-C3, Index 0-2) and the second dimension is
	 *         the block number (Index 0-3).
	 */
	public static byte[][] acToACMatrix(byte ac[]) {
		...
		if ((byte) ((ac[1] >>> 4) & 0x0F) == (byte) ((ac[0] ^ 0xFF) & 0x0F) && (byte) (ac[2] & 0x0F) == (byte) (((ac[0] ^ 0xFF) >>> 4) & 0x0F)
	...
	public static boolean isHexAnd48Byte(String hexString, Context context) {
		if (hexString.matches("[0-9A-Fa-f]+") == false) {
			// Error, not hex.
			// Toast.makeText(context, R.string.info_not_hex_data,
			// Toast.LENGTH_LONG).show();
	...
	public static boolean isHexAnd32Byte(String hexString, Context context) {
		if (hexString.matches("[0-9A-Fa-f]+") == false) {
			// Error, not hex.
			// Toast.makeText(context, R.string.info_not_hex_data,
			// Toast.LENGTH_LONG).show();
	...
	/**
	 * Check if the given block (hex string) is a value block. NXP has PDFs describing what value blocks are. Google something like
	...
	public static boolean isValueBlock(String hexString) {
		...
		if (b.length == 16) {
			...
			if ((b[0] == b[8] && (byte) (b[0] ^ 0xFF) == b[4]) && (b[1] == b[9] && (byte) (b[1] ^ 0xFF) == b[5])
					&& (b[2] == b[10] && (byte) (b[2] ^ 0xFF) == b[6]) && (b[3] == b[11] && (byte) (b[3] ^ 0xFF) == b[7])
	...
	/**
	 * Reverse a byte Array (e.g. Little Endian -> Big Endian). Hmpf! Java has no Array.reverse(). And I don't want to use Commons.Lang (ArrayUtils)
	 * form Apache....
	 * 
